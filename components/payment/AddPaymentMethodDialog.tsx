    ...
    onSubmit: async ({ value }) => {
      ...
      switch (selectedType) {
        ...
        case 'mobile_banking_app': {
          ...
          paymentDetails = JSON.stringify(details)
          if (!label) label = `${value.paymentApp.charAt(0).toUpperCase() + value.paymentApp.slice(1)} - ${value.paymentHandle}`
          break
        }
        case 'cash_in_person': {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-[500px] bg-card max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>
            {step === 'list' ? 'Payment Method' : `Add ${PAYMENT_METHODS[selectedType || '']?.name || 'Payment Method'}`}
          </DialogTitle>
        </DialogHeader>
        ...
        {step === 'list' ? (
          ...
          >
            ...
            {selectedType === 'mobile_banking_app' && (
              <MobileBankingAppForm form={form} />
            )}
            {selectedType === 'cash_in_person' && (
              <CashInPersonForm form={form} />
            )}
            {selectedType === 'cash_deposit_atm' && (
              <CashDepositAtmForm form={form} />
            )}
            ...
            <div className="flex gap-3 pt-2">
              ...
              <Button
                type="submit"
                className="flex-1 h-[56px] bg-primary hover:bg-primary/90"
                disabled={isSubmitting}
              >
                {isSubmitting ? (
                  <>
        ...
        {(field: { state: { value: string }; handleChange: (v: string) => void }) => (
          <div className="space-y-2">
            ...
            <p className="text-xs text-muted-foreground">
              The username, email, or phone number linked to your account
            </p>
          </div>
        )}
      </form.Field>
      ...
      <form.Field name="contactHandle">
        {(field: { state: { value: string }; handleChange: (v: string) => void }) => (
          <div className="space-y-2">
            ...
            />
            <p className="text-xs text-muted-foreground">
              Your Telegram username (e.g. @username) or phone number
            </p>
          </div>
        )}
      </form.Field>
